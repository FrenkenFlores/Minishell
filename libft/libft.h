/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   libft.h                                            :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: wabomina <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/05/26 18:51:04 by wabomina          #+#    #+#             */
/*   Updated: 2020/05/30 12:17:11 by wabomina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef LIBFT_H
#define LIBFT_H

# include <stdlib.h>
# include <unistd.h>
# include <stdarg.h>
# include <limits.h>

# define SPECIFIER "cspdiuxX%"
# define FLAGS "- 0"
# define BUFFER_SIZE 32

typedef struct	s_params
{
	const char	*s;
	int			i;
	int			zero;
	int			minus;
	int			width;
	int			precision;
	char		specifier;
	int			length;
}				t_params;

typedef struct		s_list
{
	void			*content;
	struct s_list	*next;
}					t_list;

int					ft_arraylen(char **array);
int					ft_atoi(const char *str);
void				ft_bzero(void *s, size_t n);
void				*ft_calloc(size_t n, size_t size);
int					ft_endwith(char *str, char *suff);
int					ft_isalnum(int c);
int					ft_isalpha(int c);
int					ft_isascii(int c);
int					ft_isdigit(int c);
int					ft_isprint(int c);
int					ft_isspace(char c);
char				*ft_itoa(int n);
char				*ft_itoa_base(size_t nb, int base);
char				*ft_itoa_unsigned(unsigned long long int n);
t_list				*ft_lstnew(void *content);
void				*ft_memccpy(void *dest, const void *src, int c, size_t n);
void				*ft_memchr(const void *s, int c, size_t n);
int					ft_memcmp(const void *arr1, const void *arr2, size_t n);
void				*ft_memcpy(void *dest, const void *src, size_t n);
void				*ft_memmove(void *dest, const void *src, size_t n);
void				*ft_memset(void *dest, int c, size_t n);
void				ft_putchar_fd(char c, int fd);
void				ft_putendl_fd(char *s, int fd);
void				ft_putnbr_fd(int n, int fd);
void				ft_putstr_fd(char *s, int fd);
char				**ft_split(char const *s, char c);
char				*ft_strchr(const char *s, int c);
int					ft_strcmp(const char *s1, const char *s2);
char				*ft_strdup(const char *str);
int					ft_strisnum(char *s);
char				*ft_strjoin(char const *s1, char const *s2);
size_t				ft_strlcat(char *dst, const char *src, size_t size);
size_t				ft_strlcpy(char *dst, const char *src, size_t size);
size_t				ft_strlen(const char *str);
char				*ft_strmapi(char const *s, char (*f)(unsigned int, char));
int					ft_strncmp(const char *s1, const char *s2, size_t n);
char				*ft_strnstr(const char *s1, const char *s2, size_t n);
char				*ft_strrchr(const char *s, int c);
char				*ft_strtrim(char const *s, char const *set);
char				*ft_substr(char const *s, unsigned int start, size_t len);
int					ft_tolower(int c);
int					ft_toupper(int c);
int					ft_printf(const char *s, ...);
void				init_data(t_params *data);
void				parse_options(t_params *data, va_list ap);
void				parse_params(t_params *data, va_list ap);
void				check_flags(t_params *data);
void				check_width(t_params *data, va_list ap);
void				check_precision(t_params *data, va_list ap);
void				ptf_ft_putchar_fd(char c, int fd, t_params *data);
void				ptf_ft_putstr_fd(char *str, int fd, t_params *data);
void				ft_print_c(t_params *data, va_list ap);
void				ft_print_s(t_params *data, va_list ap);
void				ft_print_pers(t_params *data);
void				ft_print_u(t_params *data, va_list ap);
void				ft_print_x(t_params *data, va_list ap);
void				ft_print_xupper(t_params *data, va_list ap);
void				ft_print_p(t_params *data, va_list ap);
void				ft_print_d(t_params *data, va_list ap);
void				ft_print_i(t_params *data, va_list ap);
void				checkoptions_gap(int gaps, char c, t_params *data, int option);
void				gaps(int gaps, char c, t_params *data);
void				print_nbr(int len, t_params *data, char *p);
char				flag_relate(t_params *data);
int					get_next_line(int fd, char **line);
char				*ft_strjoin_gnl(char *s1, char *s2);

#endif
